...
namespace Library.ConfigurationData.ConfigurationOrder
{
    public class ConfigurationCartUser : IEntityTypeConfiguration<T_CartUser>
    {
        public void Configure(EntityTypeBuilder<T_CartUser> builder)
        {
            ...
            builder.HasOne(x => x.t_Customer).WithMany(x => x.t_CartUsers).HasForeignKey(x => x.IDUser).OnDelete(DeleteBehavior.NoAction);
            builder.HasOne(x => x.t_Cake).WithMany(x => x.t_CartUsers).HasForeignKey(x => x.IDCake).OnDelete(DeleteBehavior.NoAction);
        }
    }
}
