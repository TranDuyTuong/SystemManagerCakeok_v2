...
namespace Library.ConfigurationData.ConfigurationOrder
{
    public class ConfigurationOrder : IEntityTypeConfiguration<T_Order>
    {
        public void Configure(EntityTypeBuilder<T_Order> builder)
        {
            ...
            builder.HasOne(x => x.t_InfomationOrder).WithMany(x => x.t_Orders).HasForeignKey(x => x.IDInfomationOrder).OnDelete(DeleteBehavior.NoAction);
            builder.HasOne(x => x.t_Customer).WithMany(x => x.t_Orders).HasForeignKey(x => x.IDUser).OnDelete(DeleteBehavior.NoAction);
            builder.HasOne(x => x.t_TypePay).WithMany(x => x.t_Orders).HasForeignKey(x => x.IDPay).OnDelete(DeleteBehavior.NoAction);
            builder.HasOne(x => x.t_StatusOrder).WithMany(x => x.t_Orders).HasForeignKey(x => x.IDStatusOrder).OnDelete(DeleteBehavior.NoAction);
        }
    }
}
